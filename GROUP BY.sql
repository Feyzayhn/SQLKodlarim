...
select urun_adi, min(urun_miktari) as minimum_urun_mik, max(urun_miktari) as maksimum_urun_mik
from manav  where urun_adi is not null 
group by urun_adi -- grupladik
order by maksimum_urun_mik -- siraladik
...
--select cinsiyet buraya ve group by cinsiyet buraya yazdigim ayni olacak
-- as ortalama_maas tabloya ek bir tablo istedik yapmasakta olur

-------------------------------------------------------------------------------------

-- Personeli, ulkelere göre ve şehirlere göre gruplayarak sorgulayın

select ulke,sehir from personel group by ulke,sehir order by ulke, sehir
...
select ulke, cinsiyet, count(*) as calisan_sayisi, round (avg(yas)) from personel
where maas>30000
group by ulke, cinsiyet
